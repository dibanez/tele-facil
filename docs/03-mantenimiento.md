...
| 6 | ¿`python3 tools/probe_deco.py` encuentra el deco? | Problema de red o el deco perdió el emparejamiento |
...
### "Cambia de canal pero siempre al mismo sitio equivocado"

El operador reordenó la rejilla. Vuelve a la Fase 5 de la guía de despliegue y
actualiza el valor `posicion:` de los scripts afectados.

### "A veces cambia al canal equivocado"

El deco fue más lento de lo previsto y se perdieron pulsaciones, así que la
cuenta de posiciones se descolocó. En `script.tv_canal`, sube el `delay` de
6 s posterior al enlace, o los 800 ms entre flechas.
...
que el deco arranque y `script.tv_send_key` reconectará solo.
